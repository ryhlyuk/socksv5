...
Server.prototype._onConnection = function(socket) {
  ...
  parser.on('error', function(err) {
    ...
    if (self.options && self.options.reqOptions) {
      reqInfo.options = self.options.reqOptions;
    }
...
function proxySocket(socket, req) {
  dns.lookup(req.dstAddr, function(err, dstIP) {
    ...
    if (req.options && req.options.outgoingAddress) {
      connectOptions.localAddress = dstSock.localAddress = req.options.outgoingAddress;
    }
